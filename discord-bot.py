    ...
    while True:
        ...
        def after_playing(error):
            ...
            try:
                ...
                print(f"Finished playing {os.path.basename(filepath)}. Deleting file.")
                if os.path.exists(filepath):
                    os.remove(filepath)
                ...
                # This is thread-safe and signals the worker to play the next track
        ...
        try:
            ...
            if local_playback_bot_enabled:
                print("Attempting to start local playback with paplay...")
                try:
                    # Reverting to subprocess.Popen as it worked before.
                    # Redirecting stdout/stderr to devnull to avoid potential blocking.
                    ...
                    print("paplay process started.")
                except Exception as e:
                    print(f"Error starting paplay subprocess: {e}")

        except Exception as e:
            print(f"Error starting playback: {e}")
            # If play fails, we must reset the event so we don't get stuck
            playback_finished.set()
